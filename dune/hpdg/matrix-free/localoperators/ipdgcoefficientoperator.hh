...
#ifndef DUNE_FUFEM_MATRIX_FREE_LOCAL_IPDG_FACE_COEFFICIENT_OPERATOR_HH
#define DUNE_FUFEM_MATRIX_FREE_LOCAL_IPDG_FACE_COEFFICIENT_OPERATOR_HH
...
namespace MatrixFree {
  ...
  /** computes (K∇φ_j, ∇u) for all j on a given element
   * 
   * The difference to the IPDGOperator also found here is,
   * that we have a (scalar) coefficient function K here.
  */
  template<class V, class GV, class Coeff, class Basis>
  class IPDGCoefficientOperator : public LocalOperator<V, GV> {
    ...
    using LF = typename std::decay_t<decltype(localFunction(std::declval<const Coeff&>()))>;
    ...
    public:
      ...
      IPDGCoefficientOperator(const Basis& b, const Coeff& coeff, int extraQuadOrder, double penalty=2.0, bool dirichlet=false) :
        basis_(b),
        coeff_(coeff),
        extraQuadOrder_(extraQuadOrder),
        penalty_(penalty),
        dirichlet_(dirichlet),
        localCoeff_(localFunction(coeff_)),
        outerLocalCoeff_(localFunction(coeff_)),
    ...
    private:
      ...
      void computeFace() {
        ...
        for (const auto& is: intersections(gv, localView_.element())) {
          ...
          outerLocalCoeff_.bind(is.outside());

          outerLocalVector_.resize(outerLocalView_.size());
          for(auto& e: outerLocalVector_)
            e=0;

          const auto& insideGeometry = is.geometryInInside();
          const auto& outsideGeometry = is.geometryInOutside();
          ...
          auto quadKey = QuadratureRuleKey(is.type(), maxOrder + extraQuadOrder_).square();
          ...
          for (size_t q = 0; q < nPoints; q++) {
            ...
            auto coeff_inside = localCoeff_(insideGeometry.global(quadPos));
            auto coeff_outside = outerLocalCoeff_(outsideGeometry.global(quadPos));
            ...
            for (auto& grad : insideRefGradients) {
              grad[0] *= coeff_inside;
            }
            ...
            for (auto& grad : outsideRefGradients) {
              grad[0] *= coeff_outside;
            }
      ...
      void computeBulk() {
        ...
        QuadratureRuleKey feQuad(localView_.element().type(), fe.localBasis().order() + extraQuadOrder_);
        ...
        for (size_t q = 0; q < nPoints; q++) {
          ...
          auto coeff_val = localCoeff_(quadPos);
          ...
          for (size_t i = 0; i < referenceGradients.size(); i++) {
            ...
            localVector_[i] += coeff_val * (gradient*duq);
          }
        }
      }
      ...
      void computeDirichletBoundaryEdge(const IS& is, const C& insideCoeffs) {
        DUNE_THROW(Dune::Exception, "Dirichlet not yet implemented");
        auto applyOp= [](const auto& op, auto& vec) {
          auto tmp = vec;
          op.mv(tmp, vec);
        };
