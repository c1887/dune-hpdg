...
//    std::vector<typename V::value_type> localDoFs_;
  ...
  DiscretePersistentBasisFunction(const Basis & basis, const TreePath& treePath, const V & coefficients, const NodeToRangeEntry& nodeToRangeEntry) :
...
auto makeDiscretePersistentBasisFunction(B&& basis, const TP& treePath, V&& vector)
{
  ...
  return DiscretePersistentBasisFunction<Basis, TP, Vector, NTREM, R>(basisPtr, treePath, vectorPtr, nodeToRangeEntryPtr);
}
