...
/** This is a modification of the DiscreteGlobalBasisFunction from dune-functions.
 * TODO: Include proper Copyright claim.
 */
...
class SubGridDiscreteGlobalBasisFunction
{
public:
  using Basis = B;
  using Vector = V;
  ...
  // Here, we assume that the leaf grid view is what we're interested in...
  using GridView = typename Basis::GridView::Grid::HostGridType::LeafGridView;
  ...
  public:
    ...
    using GlobalFunction = SubGridDiscreteGlobalBasisFunction;
    using Domain = LocalDomain;
    using Range = GlobalFunction::Range;
    using Element = GlobalFunction::Element;

    LocalFunction(const SubGridDiscreteGlobalBasisFunction& globalFunction)
    ...
    {
      // TODO: hier muss das Element aus dem Subgrid identifiziert werden,
      // an das wir binden, denn unser LocalView kennt nur Subgrid Elemente.
      // Das _Eingabeelement_ `element` ist allerdings aus dem Hostgrid.
      fineElement_ = &element;

      auto subGridElement = findSubGridElement(element);


      localView_.bind(subGridElement);
      bound_ = true;
    ...
    Range operator()(Domain x) const
    {
      ...
      x = transformToSubGridPosition(std::move(x));
      ...
      TypeTree::forEachLeafNode(localView_.tree(), [&](auto&& node, auto&& treePath) {
        ...
        for (size_type i = 0; i < localBasis.size(); ++i)
        {
          ...
          // in the class documentation of SubGridDiscreteGlobalBasisFunction.
          auto&& dimC = c.size();
          auto dimV = v.size();
  ...
  private:
    ...
    auto findSubGridElement(Element hostElement) const {
      while(!localView_.globalBasis().gridView().grid().template contains<0>(hostElement)) {
        if (!hostElement.hasFather())
          DUNE_THROW(Dune::Exception, "Can't find ancestor to element that is contained in SubGrid!");
        hostElement = hostElement.father();
      }
      return localView_.globalBasis().gridView().grid().template getSubGridEntity<0>(hostElement);
    }

    Domain transformToSubGridPosition(Domain x) const {
      // If x is from a finer element than what is stored in the localView, modify it accordingly.
      auto e = *fineElement_;
      auto hostCoarse = localView_.globalBasis().gridView().grid().template getHostEntity<0>(localView_.element());
      while (e != hostCoarse) {
        if (not e.hasFather())
          DUNE_THROW(Dune::Exception, "Didnt find father element in subgrid");
    ...
    const SubGridDiscreteGlobalBasisFunction* globalFunction_;
  ...
  SubGridDiscreteGlobalBasisFunction(B_T && basis, V_T && coefficients, NTRE_T&& nodeToRangeEntry) :
    entitySet_(basis.gridView().grid().getHostGrid().leafGridView()),
  ...
  SubGridDiscreteGlobalBasisFunction(std::shared_ptr<const Basis> basis, std::shared_ptr<const V> coefficients, std::shared_ptr<const NodeToRangeEntry> nodeToRangeEntry) :
    entitySet_(basis->gridView().grid().getHostGrid().leafGridView()),
    basis_(basis),
  ...
  friend typename Traits::DerivativeInterface derivative(const SubGridDiscreteGlobalBasisFunction& t)
  {
    DUNE_THROW(NotImplemented,"not implemented");
  }

  /**
   * \brief Construct local function from a SubGridDiscreteGlobalBasisFunction
   *
   * \ingroup FunctionImplementations
   *
   * The obtained local function satisfies the concept
   * `Dune::Functions::Concept::LocalFunction` and must be bound
   * to an entity from the entity set of the SubGridDiscreteGlobalBasisFunction
   * before it can be used.
   *
   * Notice that the local function stores a reference to the
   * global SubGridDiscreteGlobalBasisFunction. Hence calling any method
   * of the local function after the SubGridDiscreteGlobalBasisFunction
   * exceeded its life time leads to undefined behavior.
   */
  friend LocalFunction localFunction(const SubGridDiscreteGlobalBasisFunction& t)
  {
...
/**
 * \brief Construction of local functions from a temporary SubGridDiscreteGlobalBasisFunction (forbidden)
 *
 * Since a SubGridDiscreteGlobalBasisFunction::LocalFunction stores a reference
 * to the global SubGridDiscreteGlobalBasisFunction its life time is bound to
 * the latter. Hence construction from a temporary SubGridDiscreteGlobalBasisFunction
 * would lead to a dangling reference and is thus forbidden/deleted.
 *
 * \ingroup FunctionImplementations
 */
template<typename... TT>
void localFunction(SubGridDiscreteGlobalBasisFunction<TT...>&& t) = delete;



template<typename R, typename B, typename V>
auto makeSubGridDiscreteGlobalBasisFunction(B&& basis, V&& vector)
{
  ...
  return SubGridDiscreteGlobalBasisFunction<Basis, Vector, NTREM, R>(
      std::forward<B>(basis),
