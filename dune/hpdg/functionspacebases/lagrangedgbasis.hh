...
public:
  ...
  template<class TP>
  using IndexSet = LagrangeDGNodeIndexSet<GV, k, MI, TP, useTwoLevelIndex>;
...
public:
  ...
  using NodeFactory = LagrangeDGNodeFactory<GV, k, MI, useTwoLevelIndex>;
...
/** \brief Basis of a scalar k-th-order Lagrangean-DG finite element space with a real multiindex
 *
 * This implementation uses a array with two entries as multiindex where the first index denotes the element
 * and the second one the local (element-bound) index of the basis function.
 *
 * \ingroup FunctionSpaceBasesImplementations
 *
 * \tparam GV The GridView that the space is defined on
 ...
 */
template<typename GV, int k>
using TwoLevelLagrangeDGBasis = DefaultGlobalBasis<LagrangeDGNodeFactory<GV, k, std::array<size_t, 2>, true>>;
