...
namespace HPDG {
   ...
   */
  template<class Basis, class GF>
  class GaussLobattoIPDGCoefficientAssembler {
    ...
    public:
      ...
      GaussLobattoIPDGCoefficientAssembler(const Basis& b, GG&& gf, double penalty=2.0, bool dirichlet=false) :
        basis_(b),
        gf_(std::forward<GG>(gf)),
        penalty_(penalty),
    ...
    private:
      ...
      void computeFace(const E& edge, LC& matrixContainer, int outerDegree) {
        ...
        auto gf_inside = localFunction(gf_);
        const auto insideE = edge.inside();
        gf_inside.bind(insideE);
        const auto outsideE = edge.outside();
        auto gf_outside = localFunction(gf_);
        gf_outside.bind(outsideE);
        ...
        for (size_t pt=0; pt < rule.size(); ++pt)
        {
          ...
          auto gf_inside_val = gf_inside(edge.geometryInInside().global(quadPos));
          auto gf_outside_val = gf_outside(edge.geometryInOutside().global(quadPos));
      ...
      void computeDirichletFace(const E& edge, LM& localMatrix) {
        ...
        auto g_local = localFunction(gf_);
        g_local.bind(edge.inside());
        ...
        for (size_t pt=0; pt < rule.size(); ++pt)
        {
          ...
          // transform gradients
          auto gval = g_local(edge.geometryInInside().global(quadPos));
          for (size_t i=0; i<insideGradients.size(); ++i) {
            invJacobian_i.mv(insideReferenceGradients[i], insideGradients[i]);
            insideGradients[i] *= gval;
          }
          // evaluate basis functions
        ...
        for (size_t q0=0; q0 < rule_->size(); ++q0) {
          ...
          for (size_t q1=0; q1 < rule_->size(); ++q1)
          {
            ...
            for (size_t i=0; i<localSize_; ++i)
            {
              ...
              {
                double zij = g_local(quadPos) * (gradients[i] * gradients[j]) * z;
                localMatrix[i][j]+=zij;
                localMatrix[j][i]+=zij;
              }
              localMatrix[i][i]+= g_local(quadPos) * (gradients[i] * gradients[i]) * z;
            }
