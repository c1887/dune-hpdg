...
TestSuite test_mappedCache() {
  TestSuite suite;

  auto cache_from_function = HPDG::MappedCache<Range, Domain>(foo);
  ...
  double factor = 0.5;
  auto cache_from_lambda = HPDG::MappedCache<double, int>([&](int i) {
      return factor*i;
      });
  ...
  F f;
  auto cache_from_class = HPDG::MappedCache<Range, Domain>(std::move(f));
...
int main(int argc, char** argv) {
  ...
  TestSuite suite;
  suite.subTest(test_mappedCache());
  return suite.exit();
}
